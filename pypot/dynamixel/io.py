...
class DxlIO(AbstractIO):
    ...
    def get_control_table(self, ids, **kwargs):
        ...
        for id, model in zip(ids, self.get_model(ids)):
            ...
            length = controls[-1].address + controls[
                -1].nb_elem * controls[-1].length
...
_add_control('torque mode',
             address=0x46, length=1,
             models=('MX-64', ),
             dxl_to_si=dxl_to_bool,
             getter_name='is_torque_mode',
             setter_name='set_torque_mode')
...
_add_control('goal acceleration',
             address=0x49,
             models=('MX-64', ),
             dxl_to_si=dxl_to_acc,
             si_to_dxl=acc_to_dxl)
